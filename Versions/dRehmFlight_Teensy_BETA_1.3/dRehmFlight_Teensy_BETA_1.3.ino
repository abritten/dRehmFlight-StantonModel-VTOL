...
/*
  ...
  Tom Stanton for making and sharing public Youtube content on aviation and drone technology prototypes. -Drew
...
unsigned long channel_1_fs = 1000; //thro //altitude //wrong //ch3
unsigned long channel_2_fs = 1500; //ail // roll// wrong //ch1
...
//PITCH
float Kp_pitch_angle = 0.5;   //PULLBACK//Pitch P-gain - angle mode
float Ki_pitch_angle = 0.01;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.15;  //ROCKING//Pitch D-gain - angle mode (has no effect on controlANGLE2)
...
int m1_command_PWM, m2_command_PWM, m3_command_PWM, m4_command_PWM, m5_command_PWM, m6_command_PWM, throttleTop, throttleHover, throttleDescend;
...
void setup() {
  ...
  */
  ...
  throttleHover = 190; // Limited to ascending hover
  throttleDescend = 185; // Limited to descending hover
...
void loop() {
  ...
  //printLidarMotorCommands(); //Prints the values being written to the motors (expected: 120 to 250)
  //GraphLidarMotorCommands(); //(expected: 0 to 255)
  ...
  controlAlt();   //Get lidar distance and compute stabilized output
  controlANGLE(); //PID Controller - SELECT ONE: //Stabilize on angle setpoint //controlANGLE2(); //Stabilize on angle setpoint using cascaded method. Rate controller must be tuned well first! //controlRATE(); //Stabilize on rate setpoint
  controlMixer(); //Actuator mixing and scaling to PWM values //Mixes PID outputs to scaled actuator commands -- custom mixing assignments done here
  scaleCommands(); //Scales motor commands to 125 to 250 range (oneshot125 protocol) and servo PWM commands to 0 to 180 (for servo library)
  //////throttleLimit(); //Throttle limit check CH6
  throttleCut(); //Throttle cut check //Directly sets motor commands to low based on state of ch5
  commandMotors(); //Command actuators //Sends command pulses to each motor pin using OneShot125 protocol
  ...
  getCommands(); //Get vehicle commands for next loop iteration //Pulls current available radio commands
...
void controlMixer() {
  ...
  */
  /*
      ...
      //0.5 is centered servo, O is zero throttle if connecting to ESC for conventional PWM, 1 is max throttle
  ...
  */
  ...
  /*
    ...
    m1_command_scaled = thro_des + pitch_passthru * roll_PID + (1 - pitch_passthru) * yaw_PID; // + (1-pitch_passthru) *roll_PID*0.2;
    m2_command_scaled = thro_des - pitch_passthru * roll_PID - (1 - pitch_passthru) * yaw_PID; // - (1-pitch_passthru) *roll_PID*0.2;
    m3_command_scaled = 0;
    m4_command_scaled = 0;
    m5_command_scaled = 0;
    m6_command_scaled = 0;
    //0.5 is centered servo, O is zero throttle if connecting to ESC for conventional PWM, 1 is max throttle
    s1_command_scaled = (pitch_passthru * 0.5) + 0.07 - pitch_passthru * pitch_PID + pitch_passthru * yaw_PID + (1 - pitch_passthru) * roll_PID * 4;
    s2_command_scaled = 1 - ((pitch_passthru * 0.5) + 0.07 - pitch_passthru * pitch_PID - pitch_passthru * yaw_PID) + (1 - pitch_passthru) * roll_PID * 4;
    s3_command_scaled = 0.37 + 3 * pitch_PID;
    s4_command_scaled = 0;
    s5_command_scaled = 0;
    s6_command_scaled = 0;
...
void controlAlt() {
  ...
  alt_PID = constrain(alt_PID, 0.0, 0.5); //0.5

  if ((tf_alt_filtered > alt_maxLidar) || (tf_alt_filtered <= 3.0)) {
    alt_PID = 0.0;
  }
...
void scaleCommands() {
  ...
  */
  ...
  m1_command_PWM = constrain(m1_command_PWM, 125, throttleTop);
  m2_command_PWM = constrain(m2_command_PWM, 125, throttleTop);
...
void getCommands() {
  ...
  */
  ...
  if (sbus.read(&sbusChannels[0], &sbusFailSafe, &sbusLostFrame))
  {
    ...
    channel_4_pwm = sbusChannels[3] * scale + bias; //yaw//roll
...
void throttleLimit() {
  if (channel_6_pwm > 1900) {
    throttleTop = throttleHover; //125-250
  }
  else if ((channel_6_pwm > 1400) && (channel_6_pwm < 1600)) {
    throttleTop = throttleHover;
  }
  else {
    throttleTop = 250;
  }
