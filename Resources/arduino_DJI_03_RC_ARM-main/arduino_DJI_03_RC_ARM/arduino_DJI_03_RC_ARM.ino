...
void loop() {
  ...
  if ((uint32_t)(currentMillis_MSP - previousMillis_MSP) >= next_interval_MSP) {
    ...
    if ((currentMillis_MSP < (activityDetectedMillis_MSP + unarmedMillis))) {  //add teensy switch
...
void set_flight_mode_flags(bool arm) {
  if ((flightModeFlags == 0x00000002) && arm && armO3) {
    flightModeFlags = 0x00000003;  // arm
#ifdef DEBUG
    Serial.println("ARMING");
#endif
  } else if (!armO3 || ((flightModeFlags == 0x00000003) && !arm)) {
